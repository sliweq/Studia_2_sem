...
public class fcfs {
    ...
    public fcfs(ArrayList<proces> processes){
    ...
    public double[] start_simulation(){
        ...
        add_processes_queue();
        if (processes_queue != null && processes_queue.size() >0 ){
        ...
        double[] report = new double[]{averange_waiting_time,longest_waiting_time};
        return report;
    }   
    ...
    public void add_processes_queue(){ 
        for(int y = all_procesess.size()-1; y>=0;y--){
            proces tmp_proces = all_procesess.get(y);
            if(tmp_proces.getCzas_pojwania() == current_time){
                processes_queue.add(tmp_proces); 
                all_procesess.remove(tmp_proces);
            }
        }
