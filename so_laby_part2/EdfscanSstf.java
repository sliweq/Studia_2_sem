...
public class EdfscanSstf {
    ...
    public EdfscanSstf(int allRequestsAmount,int allRealTime, ArrayList<Request> allRequests ){
    ...
    public void startSimulatio(){
        ...
        System.out.println("edfscansstf");
    ...
    public void allRequestsUpdate(){ 
        while(allRequests.size() != 0 && allRequests.get(0).getArrivalTime() == currentTime){
            ...
            if(allRequests.get(0).getRealTimeRequest()){
                if(priorityRequest == null){
                    if(Math.abs(allRequests.get(0).getPlaceOnDrive()-armPosition) <= allRequests.get(0).getDeadLine() ){
                        priorityRequest = allRequests.get(0);
                    }
                }
                else{
                    if(Math.abs(allRequests.get(0).getPlaceOnDrive()-armPosition) < Math.abs(priorityRequest.getPlaceOnDrive()-armPosition)){
                        if(Math.abs(allRequests.get(0).getPlaceOnDrive()-armPosition) <= allRequests.get(0).getDeadLine() ){
                            priorityRequest = allRequests.get(0);
                        }
                    }
        ...
        if(requestsQueue.size()!=0 && priorityRequest == null){
            for(int y = requestsQueue.size()-1; y >= 0; y--){
                if(requestsQueue.get(y).getPlaceOnDrive() == armPosition){
                    ...
                    if(requestsQueue.get(y).getRealTimeRequest()){
                        realTimeDone +=1;
                    }
            ...
            if(armPosition == priorityRequest.getPlaceOnDrive()){
                ...
                for(Request req: requestsQueue){
                    if(req.isRealTimeRequest()){
                        if(priorityRequest == null){
                            if(Math.abs(req.getPlaceOnDrive()-armPosition) <= req.getDeadLine() ){
                                priorityRequest = req;
                            }

                        }
                        else if(Math.abs(req.getPlaceOnDrive()-armPosition) < Math.abs(priorityRequest.getPlaceOnDrive()-armPosition)){
                            if(Math.abs(req.getPlaceOnDrive()-armPosition) <= req.getDeadLine() ){
                                priorityRequest = req;
                            }
