...
public class Fcfs {
    ...
    public Fcfs(int allRequestsAmount,int allRealTime, ArrayList<Request> allRequests ){
    ...
    public void updateArm(){
        if(requestsQueue.size() != 0){
            if(armPosition - requestsQueue.get(0).getPlaceOnDrive() <= 0 ){
                if (armDirection != 1){
                    armMoves +=1;
                }
                armDirection = 1;
    ...
    public void updateArmAndRequest(){
        if(requestsQueue.size()!=0){
            if(armPosition == requestsQueue.get(0).getPlaceOnDrive()){
